...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
  ...
  np->state = RUNNABLE;
  np->ticks = 0;
  settickets(pid, curproc->tickets);
  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p=ptable.proc; p < &ptable.proc[NPROC]; p++) {
        ...
        cprintf("check\n");
        t_tickets = ticketCount();
        cprintf("%d\n",t_tickets);
        p = hold_lottery(t_tickets);
        c->proc = p;
        switchuvm(p);
        p->state = RUNNING;
...
struct proc *hold_lottery(int total_tickets) {
    ...
    //*TODO*// 
    uint winner_ticket_number = random_number % (total_tickets + 1); // Ensure that it is less than total number of tickets.
    ...
    if (counter < winner_ticket_number) continue;
...
int getpinfo(struct pstat *p) {
    ...
    for (pro=ptable.proc; pro < &ptable.proc[NPROC]; pro++) {
        ...
        p->tickets[i] = pro->tickets;
        p->pid[i] = pro->pid;
        p->ticks[i] = pro->ticks;
        p->inuse[i] = pro->state;
        i = i + 1;
        }
        p->tickets[i] = t_tickets;
        release(&ptable.lock);
        return 0;
    }
